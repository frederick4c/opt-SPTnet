...
def _read_timing_csv(path):
    """Return a list of row dicts from one epoch_timing.csv."""
    rows = []
                ...
                rows.append(
                    {
                        ...
                        "val_seconds": float(row.get("val_seconds", "nan") or "nan"),
                    }
                )
...
def discover(results_root, warmup_epochs):
    ...
    for path in paths:
        ...
        rows = _read_timing_csv(path)
        steady = [r["train_seconds"] for r in rows[warmup_epochs:]]
        if not steady:
...
def main():
    ...
    print(f"Scanning {args.results_root} (dropping {args.warmup_epochs} warmup epoch(s) per run)...")
    configs = discover(args.results_root, args.warmup_epochs)
    ...
    md = ["# SPTnet training-speed benchmark", "",
          f"Steady-state per-epoch training time (warmup epochs dropped: {args.warmup_epochs}; "
    ...
    # Decomposition: each disabled-optimization config vs full-new.
    decomp = [c for c in sorted(boot) if c not in (args.old_config, args.new_config)]
    ...
    if _HAVE_MPL:
        ...
        axp.set_ylabel("train seconds / epoch (steady-state)")
        axp.set_title("SPTnet per-epoch training time by config")
        plt.setp(axp.get_xticklabels(), rotation=30, ha="right")
        fig.tight_layout()
        plot_path = os.path.join(out_dir, "per_epoch_times.png")
